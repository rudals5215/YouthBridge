...
import { addBookmark, removeBookmark, checkBookmark } from "../apis/bookmarkApi";
import { useAuthStore } from "../stores/authStore";
import "./PolicyDetail.css";

// 카테고리별 색상 매핑
const CATEGORY_META: Record<string, { bg: string; color: string; icon: string }> = {
  "취업지원": { bg: "var(--cat-job-bg)",     color: "var(--cat-job-color)",     icon: "ri-briefcase-line" },
  "주거지원": { bg: "var(--cat-housing-bg)", color: "var(--cat-housing-color)", icon: "ri-home-4-line" },
  "창업지원": { bg: "var(--cat-startup-bg)", color: "var(--cat-startup-color)", icon: "ri-rocket-line" },
  "교육지원": { bg: "var(--cat-edu-bg)",     color: "var(--cat-edu-color)",     icon: "ri-graduation-cap-line" },
  "생활지원": { bg: "var(--cat-life-bg)",    color: "var(--cat-life-color)",    icon: "ri-heart-line" },
  "문화지원": { bg: "var(--cat-culture-bg)", color: "var(--cat-culture-color)", icon: "ri-palette-line" },
  "금융지원": { bg: "var(--cat-finance-bg)", color: "var(--cat-finance-color)", icon: "ri-bank-line" },
  "건강지원": { bg: "var(--cat-health-bg)",  color: "var(--cat-health-color)",  icon: "ri-heart-pulse-line" },
};
      ...
      <div className="detail-container">
        ...
        <header className="detail-header">
          <div className="detail-icon-wrap" style={{ background: meta.bg }}>
            <div className="detail-icon" style={{ background: `${meta.color}20`, color: meta.color }}>
              <i className={meta.icon} />
            </div>
          </div>

          <div className="detail-badges">
            <span className="badge badge-category" style={{ color: meta.color, background: meta.bg }}>
              {policy.category}
            </span>
            <span className="badge badge-region">{regionLabel}</span>
            <span className={`badge badge-status ${policy.status.toLowerCase()}`}>
              {STATUS_LABEL[policy.status] ?? policy.status}
            </span>
            {policy.dday !== null && policy.dday !== undefined && !policy.expired && (
        ...
        <footer className="detail-actions">
          ...
          {policy.applyUrl ? (
            <a href={policy.applyUrl} target="_blank" rel="noreferrer" className="btn-apply">
