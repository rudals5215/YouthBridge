          ...
          <div className="signup-step-indicator">
            {STEP_LABELS.map((label, i) => (
              <div key={label} className="signup-step-item">
                <div
                  className={`signup-step-dot ${i < step ? "done" : i === step ? "active" : ""}`}
                >
                  {i < step ? <i className="ri-check-line" /> : i + 1}
                </div>
                {i < STEP_LABELS.length - 1 && (
                  <div
                    className={`signup-step-connector ${i < step ? "done" : ""}`}
                  />
                )}
              </div>
            ))}
        ...
        <div className="auth-form-wrap">
          ...
          <div className="signup-mobile-progress">
            {STEP_LABELS.map((_, i) => (
              <div
                key={i}
                className={`signup-progress-bar ${i <= step ? "active" : ""}`}
              />
            ))}
          </div>
          ...
          {step === 2 ? (
            <div className="signup-complete">
              ...
              <button
                className="auth-submit-btn outline"
                onClick={() => navigate("/")}
              >
                홈으로 이동
              </button>
                ...
                {step === 0 && (
                  <>
                    ...
                    <div className="auth-field">
                      <label>비밀번호 확인 *</label>
                      <div
                        className={`auth-input-wrap ${confirmPw && !pwMatch ? "error" : ""}`}
                      >
                        <i className="ri-lock-line" />
                        <input
                          type="password"
